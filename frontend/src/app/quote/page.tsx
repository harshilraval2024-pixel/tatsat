...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    description: `Request a rooftop solar quote from ${cfg.name}. Upload your electricity bill for accurate savings modelling across Gujarat and India.`,
  };
}
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 rounded-3xl border border-white/10 bg-bg-elevated/70 p-6 text-center text-sm text-muted">
        Prefer voice? Call{" "}
        <a className="font-semibold text-primary hover:underline" href={`tel:${cfg.phoneTel}`}>
          {cfg.phoneDisplay}
        </a>{" "}
        · {cfg.hours}
      </div>
